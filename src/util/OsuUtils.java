...
import static osu.HitObject.TIMING_UNCERTAINTY_MS;
...
public class OsuUtils {
    ...
    public static Set<Long> getDistinctStartTime(ArrayList<HitObject> sourceList, ArrayList<HitObject> targetList) {
        Set<Long> output = new TreeSet<>();
        for (HitObject ho : sourceList) {
            output.add(ho.getStartTime());
        }
        for (HitObject ho : targetList) {
            if (output.contains(ho.getStartTime() + TIMING_UNCERTAINTY_MS) || output.contains(ho.getStartTime() - TIMING_UNCERTAINTY_MS)) {
                log.warn("skipping adding timing {} due to {} ms gap", ho.getStartTime(), TIMING_UNCERTAINTY_MS);
            } else {
                output.add(ho.getStartTime());
            }
        }
        log.info("distinct times from both diff {}", output);
        return output;
    }
    ...
    public static ArrayList<HitObject> getChordByTime(ArrayList<HitObject> hitObjects, long startTime) {
        ...
        for (HitObject ho : hitObjects) {
            if (ho.isStartTimeAboutEquals(startTime)) {
                output.add(ho.clone());
            }
        }
    ...
    public static ArrayList<HitObject> sortChordByHSType(ArrayList<HitObject> hitObjects) {
        ...
        defaultHS.sort(HitObject.ColumnComparator);
        output.addAll(defaultHS);
        wavHS.sort(HitObject.ColumnComparator);
        output.addAll(wavHS);
    ...
    public static int getDefaultHSChordSizeForTime(ArrayList<HitObject> hitObjects, long startTime) {
        int size = 0;
        for (HitObject ho : hitObjects) {
            if (ho.isStartTimeAboutEquals(startTime) && ho.getWhistle_finish_clap() != 0 && !ho.isWAV_HS()) {
                size++;
            }
        }
        return size;
